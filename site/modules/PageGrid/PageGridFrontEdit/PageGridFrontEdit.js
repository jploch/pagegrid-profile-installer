function PageGridFrontEditInit($) {
	...
	function inlineCancelClickEvent() {
		if ($('.pw-changed').length > 0) {
			if (confirm(ProcessWire.config.PageGridFrontEdit.labels.cancelConfirm)) {
	...
	function inlineSaveClickEvent() {
		...
		edited.each(function () {
			...
			if (t.hasClass('pw-edit-InputfieldCKEditor')) {
				...
				// JP added if(editor) check to work with ajax items
				if (editor) {
		...
		$.post(ProcessWire.config.PageGridFrontEdit.pageURL, postData, function (data) {
			btnSaving.hide();
			...
			if (data.status > 0) {
				...
				// JP only show relevant errors
				if(data.error != 'No changes to save') {
					alert(data.error);
				}
	...
	function init() {
		...
		if (width < 10) loadCSS(ProcessWire.config.PageGridFrontEdit.files.fa);
		test.hide();

		// load PageGridFrontEdit.css
		loadCSS(ProcessWire.config.PageGridFrontEdit.files.css);

		$('body').addClass('pw-' + ProcessWire.config.PageGridFrontEdit.adminTheme);
		...
		if ($('.pw-edit-InputfieldCKEditor').length) {
			jQuery.getScript(ProcessWire.config.PageGridFrontEdit.files.ckeditor, function () {
				jQuery.getScript(ProcessWire.config.PageGridFrontEdit.files.modal, function () {
			...
			jQuery.getScript(ProcessWire.config.PageGridFrontEdit.files.modal)
				.fail(function (jqxhr, settings, exception) {
					alert('failed to load modal.js: ' + exception);
				});
		}

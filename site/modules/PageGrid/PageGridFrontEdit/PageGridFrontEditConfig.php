...
class PageGridFrontEditConfig extends ModuleConfig
{
	...
	public function getDefaults()
	{
		return array(
			...
			'inlineEditFieldsUpload' => array(), // JP file fields that are inline-editable
			'placeholderText' => 'Click twice to edit this text', // JP placeholder text
			'inlineLimitPage' => 0, // 1=limit editor to rendered page only, 0=edit for any page 
	...
	public function getInputfields()
	{
		...
		$fieldset->label = $this->_('Option A: Inline editor settings');
		$fieldset->description = $this->editHelpText;
		$fieldset->description .= ' ' . $this->_('These fields will become editable on the front-end, directly in the page, simply by checking the boxes below.');
		$fieldset->description .= ' ' . $this->_('**Be careful with this option:** If you are outputting the value of a field in more than one place on a page, you should instead use [Option B, C or D](https://processwire.com/api/modules/front-end-editing/).');
		...
		$f->icon = 'font';
		$f->label = $this->_('Text fields');
		$f->optionColumns = 3;
		foreach ($fields as $field) {
			$label = $field->name;
			if ($label == 'title') $label .= ' ' . $this->_('(not recommended)');
			if ($label != 'pg_settings') $f->addOption($field->id, $label);
		}
		// $f->attr('value', $this->inlineEditFields);
		$fieldset->add($f);

		//JP files upload fields
		...
		//END JP add cutom text for new items
	...
	public function fieldHelpInputfields(InputfieldWrapper $fieldset, Field $field)
	{
		...
		$module = $this->wire('modules')->get('PageGridFrontEdit');
		$fieldset->description = $this->editHelpText;
		...
		$fieldset->description .= ' ' . sprintf(
			$this->_('Required permissions are [page-edit](%s) and [page-edit-front](%s).'),
		...
		if ($module->inlineSupported($field)) {
			...
			$href = "{$this->config->urls->admin}module/edit?name=PageGridFrontEdit&modal=1&";
			$f->value =
